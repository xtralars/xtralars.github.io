...
async function authFirebase(email, password, username, isRegister) {
  try {
    ...
    if (isRegister) {
      // Set the displayName to the provided username during user creation
...
function updateUserScore(points, username) {
  const userScoreRef = ref(leaderboardRef, username);
  ...
  return get(userScoreRef)
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        return set(userScoreRef, { score: currentScore + points });
      } else {
...
function displayLeaderboard() {
  ...
  function checkGuess() {
      ...
      if (inputGuess === correctAnswer.toLowerCase()) { // Compare user's input with the correct answer
        ...
        triggerConfetti();
        revealAnswerToPage();
        updateUserScore(userPoints, username);
      } else {
